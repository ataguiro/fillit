...
/*   Updated: 2016/11/22 09:20:25 by ataguiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		*ft_remove_newlines(char *str)
{
	int		i;
	int		j;
	char	*new;
...
static t_fillit	*ft_get_pos(char **tab)
{
	t_fillit	*new;
	t_rec		g;
...
static void		ft_correct_pos(t_fillit *pos, int nb)
{
	int		i;
	int		n;
...
t_fillit		*ft_full_it(char *file, t_info *l)
{
	...
	buf = ft_filetobuff(file);
	forbuf = ft_remove_newlines(buf);
	...
	linear = ft_get_pos(te2);
	ft_correct_pos(linear, l->n);
	ft_fill_some(&linear, l->n);
	return (linear);
}
