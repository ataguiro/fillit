...
/*   Updated: 2016/11/22 09:19:08 by ataguiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fillit.h"

static void	ft_set_it_once(int n, char **map, t_fillit *f, int p)
{
...
static void	ft_erase_it(int n, char **map, t_fillit *f, int p)
{
...
static void	ft_display(int n, char *map)
{
	write(1, map, (n + 1) * n);
}

static int	ft_fill_it(char **map, t_fillit *f, t_info *l, int *p)
{
...
int			ft_resolve(t_fillit **f, t_info *l, char **map)
{
	...
	while (g.nb != g.max)
	{
		if (ft_fill_it(map, &((*f)[g.nb]), l, &g.p))
		{
			g.nb++;
			g.p = -1;
		}
		else
		{
			...
			if (g.nb == -1)
				return (0);
			g.p = (*f)[g.nb].p;
			ft_erase_it(l->min, map, &((*f)[g.nb]), g.p);
		}
	}
	ft_display(l->min, *map);
	return (1);
}
